...
function monthsElapsed(startDateStr) {
  ...
  let months = (now.getFullYear() - s.getFullYear()) * 12 + (now.getMonth() - s.getMonth()) + 1;
...
function tenantBalance(t) {
  ...
  const charged = monthsElapsed(t.StartDate) * Number(t.MonthlyRent || 0);
...
function openTenantProfile(id) {
  ...
  $("#tpSub").textContent = `${propertyName(t.PropertyID)} · Spot ${t.SpotLabel || "—"} · ${fmt(t.MonthlyRent)}/mo`;
...
$("#btnAddTenantTop").onclick = () => {
  ...
  $("#atName").value = ""; $("#atPhone").value = ""; $("#atSpot").value = "";
  $("#atRent").value = ""; $("#atStart").value = todayStr();
  renderPropertyOptions();
  openSheet("sheetAddTenant");
};
  ...
  try {
    if (editId) {
      await api("updateTenant", {
        id: editId, name, phone: $("#atPhone").value,
        spotLabel: $("#atSpot").value, monthlyRent: rent
      });
      toast("Tenant updated");
    } else {
      await api("addTenant", {
        name, phone: $("#atPhone").value, propertyId,
        spotLabel: $("#atSpot").value, monthlyRent: rent, startDate: start
      });
      toast("Tenant added");
    }
...
async function generateReceipt({ mode, tenant, amount, date, payMode, note }) {
  ...
  rows.push(["Monthly rent", fmt(tenant.MonthlyRent)]);
  if (isPaid) {
